...
async function main() {
  const project = 'gamba-league',
    options = { project, nonInteractive: true };
  setActiveAccount(
    options,
    selectAccount('hood.travis98@gmail.com', process.cwd()),
  );
  ...
  for (const name of [...callable, 'closeLeagueWeeks', 'syncFootballScores', 'syncSundayFootballScores']) {
    ...
    if (callable.includes(name)) {
      const r = await fetch(f.serviceConfig.uri, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ data: {} }),
      });
      const body = await r.json();
      if (r.status !== 401 || body.error?.status !== 'UNAUTHENTICATED')
        throw Error(name + ' failed the authentication smoke check.');
    }
    console.log(
      name +
        ': ACTIVE' +
        (callable.includes(name) ? ', authentication enforced' : ''),
    );
  }
  const scheduler = new Client({
    urlPrefix: 'https://cloudscheduler.googleapis.com',
    auth: true,
  });
  ...
  for (const [name, schedule] of [
    ...
    const job = (
      await scheduler.get(jobBase + `firebase-schedule-${name}-us-central1`)
    ).body;
    if (
      job.state !== 'ENABLED' ||
      ...
      throw Error(name + ' schedule is not ready.');
  ...
  if (
    !['2026-09-08', '2026-09-09'].includes(
      before.fields?.startDate?.stringValue,
    ) ||
    !before.fields?.commissionerUids?.arrayValue?.values?.length
  )
    throw Error('League configuration is incomplete.');
  if (process.argv.includes('--verify-only')) {
    console.log(
      'Verification complete. No settings changed or jobs triggered.',
    );
    return;
  }
  ...
  if (!names.every((n) => after.fields?.[n]?.booleanValue === true))
    throw Error('Activation verification failed.');
  console.log(
    'Verified: betting, data imports and automatic settlement enabled.',
  );
  await scheduler.post(
    jobBase + 'firebase-schedule-syncFootballScores-us-central1:run',
    {},
  );
