...
function fixture(status = 'pending') {
  const records = new Map([
    ['bets/pick', { uid: 'player', username: 'Player', selection: 'Pick', status, stake: 400, week: 1, paid: 0, startsAt: 0 }],
    ['members/player', { balance: 17600 }],
  ]);
  let sequence = 0;
  const db = {
    doc: path => ({ path }),
    ...
    runTransaction: async fn => {
      ...
      const result = await fn({
        get: async ref => ({ ref, exists: staged.has(ref.path), data: () => staged.get(ref.path) }),
        update: (ref, data) => staged.set(ref.path, { ...staged.get(ref.path), ...data }),
        create: (ref, data) => { assert.equal(staged.has(ref.path), false); staged.set(ref.path, data); },
...
test('pending deletion refunds stake, reverses weekly stake and retains history; retry does not refund twice', async () => {
  ...
  const ledger = [...records].filter(([path]) => path.startsWith('ledger/')).map(([, data]) => data);
...
test('other players and settled bets cannot be deleted', async () => {
  const { db, records } = fixture();
  await assert.rejects(deletePendingBet(db, 'other', 'pick'), /own bets/);
  assert.equal(records.get('members/player').balance, 17600);
  for (const status of ['won', 'lost', 'push', 'void']) {
    const f = fixture(status);
    await assert.rejects(deletePendingBet(f.db, 'player', 'pick'), /Only pending/);
    assert.equal(f.records.has('bets/pick'), true);
    assert.equal(f.records.get('members/player').balance, 17600);
  }
});

test('missing and invalid bet IDs are rejected', async () => {
  const { db } = fixture();
  for (const id of ['', null, 'bets/pick']) await assert.rejects(deletePendingBet(db, 'player', id), /Invalid bet/);
